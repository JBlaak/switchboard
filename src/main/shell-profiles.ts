...
let _shellProfiles: ShellProfile[] | null = null;
export function getShellProfiles(): ShellProfile[] {
  if (!_shellProfiles) _shellProfiles = discoverShellProfiles();
  return _shellProfiles;
}

export function resolveShell(profileId?: string | null): ShellProfile {
...
export function quoteArgvForShell(shellPath: string, argv: unknown[]): string {
  return argv.map(a => quoteArgForShell(shellPath, a)).join(' ');
}

// Returns spawn args appropriate for the resolved shell
export function shellArgs(shellPath: string, cmd?: string | null, extraArgs?: string[] | null): string[] {
  const base = path.basename(shellPath).toLowerCase();
