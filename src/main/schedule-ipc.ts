// schedule-ipc.ts — IPC handlers and helpers for scheduled task creation
...
import {
  parseFrontmatter, createScheduleSession, buildScheduleCommand,
} from './schedule-runner.js';
import type { Schedule, ScheduleCli, ScheduleLogger, RunScheduledCommand } from './schedule-runner.js';
...
export function ensureScheduleCreatorCommand(): void {
  try {
...
export function init(log: ScheduleLogger, runCommand: RunScheduledCommand): void {
  ...
  ipcMain.handle('create-schedule-session', (_event, projectPath: string) => {
    try {
      ensureScheduleCreatorCommand();
      const commandPath = path.join(SCHEDULE_COMMANDS_DIR, 'create-switchboard-schedule.md');
  ...
  ipcMain.handle('run-schedule-now', (_event, filePath: string) => {
    try {
      ...
      const schedule: Schedule = {
        ...
        name: typeof meta.name === 'string' ? meta.name : path.basename(filePath),
        cron: typeof meta.cron === 'string' ? meta.cron : '* * * * *',
        slug: typeof meta.slug === 'string'
          ? meta.slug
          : path.basename(filePath, '.md').replace(/^schedule-/, ''),
        cli: (typeof meta.cli === 'object' ? meta.cli : {}) as ScheduleCli,
        prompt: body,
      };
