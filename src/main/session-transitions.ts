...
export interface TransitionContext {
  ...
  rekeyMcpServer: (oldId: string, newId: string) => void;
}

let PROJECTS_DIR: string;
let activeSessions: ActiveSessionMap;
let getMainWindow: TransitionContext['getMainWindow'];
let log: TransitionLogger;
let rekeyMcpServer: TransitionContext['rekeyMcpServer'];

export function init(ctx: TransitionContext): void {
...
function readNewSessionSignals(filePath: string): NewSessionSignals {
  try {
...
export function detectSessionTransitions(folder: string): void {
  ...
  for (const [sessionId, session] of [...activeSessions]) {
    ...
    const knownJsonlFiles = session.knownJsonlFiles;
    const newFiles = currentFiles.filter(f => !knownJsonlFiles.has(f));

    if (newFiles.length > 0) log.debug(`[detect] session=${sessionId} forkFrom=${session.forkFrom||'none'} folder=${folder} newFiles=${newFiles.length} knownCount=${knownJsonlFiles.size} currentCount=${currentFiles.length}`);
