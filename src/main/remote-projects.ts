...
import { isWindows, shellArgs, quoteArgvForShell } from './shell-profiles.js';
import type { RemoteConfig } from '../shared/types.js';

/** A parsed ssh:// path. Unlike RemoteConfig, `dir` may be absent. */
...
function expandRemoteDir(dir: string): string {
...
export function buildSshSpawn(
  sshArgv: string[],
  { shell, shellExtraArgs = [], windows = isWindows }: SshSpawnOptions = {},
