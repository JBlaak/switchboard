...
// getFolderIndexMtimeMs moved to session-cache.ts
import {
  startMcpServer, shutdownMcpServer, shutdownAll as shutdownAllMcp,
  resolvePendingDiff, rekeyMcpServer, cleanStaleLockFiles,
} from './mcp-bridge.js';
import { fetchAndTransformUsage } from './claude-auth.js';
import type { ActiveSession, ActiveSessionMap, RemoteSession, RemoteState } from './session-types.js';
import type {
  GlobalSettings, Project, RemoteProjectSetting, RemoteStatusPayload,
  SearchType, SessionOptions,
} from '../shared/types.js';

// SWITCHBOARD_DATA_DIR isolates a dev/test instance from the installed app:
// db.ts puts switchboard.db under it, and pointing userData there gives the
...
// Dev-only live reload. Left as a bare require so the dependency stays out of
// the packaged bundle's import graph; esbuild keeps it external.
...
// Ending a session → session-stop.ts
import {
  init as initSessionStop, isPtyAlive, signalSessionTree, retireSession,
  stopSessionTree, findActiveSession,
} from './session-stop.js';
// Shell profiles → shell-profiles.ts
import {
  discoverShellProfiles, getShellProfiles, resolveShell, isWindows, isWslShell,
  windowsToWslPath, shellArgs, quoteArgvForShell,
} from './shell-profiles.js';
import { startScheduler } from './schedule-runner.js';
import { encodeProjectPath } from '../shared/encode-project-path.js';
import {
  ...
  MAX_RETRIES as REMOTE_MAX_RETRIES,
...
if (app.isPackaged || process.env.FORCE_UPDATER) {
  ...
  const sendUpdaterEvent = (type: string, data?: unknown): void => {
...
function windowChromeOptions(): Partial<BrowserWindowConstructorOptions> {
...
/** Persisted window geometry, as saved into the `global` settings blob. */
interface WindowBounds { x?: number; y?: number; width: number; height: number }

function createWindow(): void {
  // Restore saved window bounds
  const savedBounds = getSetting<GlobalSettings>('global')?.windowBounds as WindowBounds | undefined;
  let bounds: { width: number; height: number } = { width: 1400, height: 900 };
  ...
  if (savedBounds && savedBounds.width && savedBounds.height) {
    ...
    const savedX = savedBounds.x;
    const savedY = savedBounds.y;
    if (savedX != null && savedY != null) {
      const displays = screen.getAllDisplays();
      const onScreen = displays.some(d => {
        const b = d.bounds;
        return savedX >= b.x - 100 && savedX < b.x + b.width &&
               savedY >= b.y - 100 && savedY < b.y + b.height;
      });
      if (onScreen) {
        restorePosition = { x: savedX, y: savedY };
      }
    }
  }
  ...
  const saveBounds = () => {
    ...
    boundsTimer = setTimeout(() => {
      ...
      const global = getSetting<GlobalSettings>('global') || {};
...
import * as sessionCache from './session-cache.js';
sessionCache.init({
  PROJECTS_DIR,
  activeSessions,
...
ipcMain.handle('add-remote-project', (_event, config) => {
  ...
  const remote = {
    user: String(config.user), host: String(config.host),
    port: Number(config.port) || 22, dir: dir ?? '', sessions: [],
  } satisfies RemoteProjectSetting;
  const projectPath = remoteProjectPath(remote);
  const global = getSetting<GlobalSettings>('global') || {};
  const list = global.remoteProjects || [];
  if (list.some((r: RemoteProjectSetting) => remoteProjectPath(r) === projectPath)) return { error: 'This remote is already added.' };
  list.push(remote);
...
ipcMain.handle('refresh-stats', async () => {
  ...
  const globalSettings = getSetting<GlobalSettings>('global') || {};
  const statsProfileId = (globalSettings.shellProfile as string | undefined) || SETTING_DEFAULTS.shellProfile;
  ...
  function runClaude(
    args: string,
    { timeoutMs = 15000, waitFor = null }: { timeoutMs?: number; waitFor?: RegExp | null } = {},
  ): Promise<string> {
...
interface MdFile { filename: string; filePath: string; modified: string }

function scanMdFiles(dir: string): MdFile[] {
...
// --- Scheduled tasks ---
import * as scheduleIpc from './schedule-ipc.js';
...
ipcMain.handle('get-shell-profiles', () => {
  // NOTE: this used to assign `_shellProfiles = null` to "refresh on each
  // request", but that name belongs to shell-profiles.ts and was never in
  // scope here — the assignment silently created a global and the cache was
  // never cleared. Behaviour is preserved (the list is discovered once per
  // launch); re-discovering per request would shell out to `wsl.exe --list`
  // on Windows every time the settings panel opens.
...
ipcMain.handle('get-effective-settings', (_event, projectPath) => {
  ...
  const project = projectPath
    ? (getSetting<Record<string, unknown>>('project:' + projectPath) || {})
    : {};
  const effective: Record<string, unknown> = { ...SETTING_DEFAULTS };
  const globalRecord = global as Record<string, unknown>;
  for (const key of Object.keys(SETTING_DEFAULTS)) {
    if (globalRecord[key] !== undefined && globalRecord[key] !== null) {
      effective[key] = globalRecord[key];
    }
...
ipcMain.handle('archive-session', (_event, sessionId: string, archived: boolean) => {
  const val = archived ? 1 : 0;
  setArchived(sessionId, Boolean(archived));
  return { archived: val };
});
...
function wirePty(session: ActiveSession, sessionId: string, ptyProcess: pty.IPty): void {
  ptyProcess.onData((data: string) => {
    const currentId = session.realSessionId || sessionId;

    if (isRemoteSession(session)) noteRemoteOutput(sessionId, session, data);
  ...
  ptyProcess.onExit(({ exitCode, signal }: { exitCode: number; signal?: number }) => {
    ...
    if (isRemoteSession(session) && handleRemoteExit(sessionId, session, exitCode, signal)) return;
...
/** Narrowing guard: lets the ssh helpers below demand a remote session. */
function isRemoteSession(session: ActiveSession | undefined | null): session is RemoteSession {
  return !!session && !!session.remote;
}

function sendRemoteStatus(sessionId: string, status: RemoteStatusPayload): void {
  const session = activeSessions.get(sessionId);
...
function spawnRemotePty(sessionId: string, r: RemoteState): pty.IPty {
...
function noteRemoteOutput(sessionId: string, session: RemoteSession, data: string): void {
...
function handleRemoteExit(sessionId: string, session: RemoteSession, exitCode: number, signal?: number): boolean {
  const r = session.remote;
...
function scheduleRemoteReconnect(sessionId: string, session: RemoteSession, reason: string): boolean {
  const r = session.remote;
  r.attempt += 1;
...
function reconnectRemoteNow(sessionId: string): void {
  const session = activeSessions.get(sessionId);
  ...
  let ptyProcess;
  try {
    ptyProcess = spawnRemotePty(sessionId, session.remote);
  } catch (err) {
    log.error(`[remote] session=${sessionId} respawn failed: ${(err as Error).message}`);
    scheduleRemoteReconnect(sessionId, session, (err as Error).message);
    return;
  }
...
function isRemoteReconnecting(session: ActiveSession | undefined): boolean {
...
function markRemoteDisconnected(session: ActiveSession | undefined): void {
  if (!session || !session.remote) return;
  session.remote.userDisconnected = true;
  if (session.remote.timer) clearTimeout(session.remote.timer);
...
ipcMain.handle('open-terminal', async (_event, sessionId, projectPath, isNew, sessionOptions) => {
  ...
  const effectiveProfileId = (() => {
    ...
    if (global.shellProfile !== undefined && global.shellProfile !== null) profileId = String(global.shellProfile);
    if (project.shellProfile !== undefined && project.shellProfile !== null) profileId = String(project.shellProfile);
    return profileId;
  })();
  ...
  try {
    if (isRemote) {
      ...
      ptyProcess = spawnRemotePty(sessionId, remoteState);
      ...
      setTimeout(() => {
        // _isDisposed is node-pty internal state with no public equivalent;
        // reading it is what stops the shim being written into a dead PTY.
        if (!(ptyProcess as pty.IPty & { _isDisposed?: boolean })._isDisposed) {
          try {
      ...
      if (sessionOptions?.mcpEmulation !== false) {
        try {
          ...
          log.error(`[mcp] Failed to start MCP server for ${sessionId}: ${(err as Error).message}`);
        }
      }
...
import * as sessionTransitions from './session-transitions.js';
...
if (!gotSingleInstanceLock) {
  ...
  app.whenReady().then(() => {
    ...
    function runScheduleCommand(claudeArgv: string[], cwd: string, name: string, onDone: () => void): void {
      const globalSettings = getSetting<GlobalSettings>('global') || {};
      const profileId = (globalSettings.shellProfile as string | undefined) || SETTING_DEFAULTS.shellProfile;
      ...
      child.on('error', (err) => {
        log.error(`[schedule] ${name} error:`, (err as Error).message);
        if (onDone) onDone();
      });
    }
