...
/** Which buttons a toolbar should carry. */
...
/** The panels a markdown preview toggle switches between. */
export interface MarkdownPreviewTargets {
 ...
 */
export function flashButtonText(btn: HTMLElement, text: string, duration = 1200): void {
  if (btn.classList.contains('fp-icon-btn')) {
    // Icon button — flash color instead of replacing content
    btn.style.color = 'var(--sb-green)';
    btn.style.borderColor = 'rgba(46,182,125,0.4)';
...
export function toggleMarkdownPreview(
  { editorEl, previewEl, toggleBtn, editorView, isPreview, storageKey }: MarkdownPreviewTargets,
): boolean {
  if (!isPreview) {
 ...
 */
export function createViewerToolbar(opts: ViewerToolbarOptions = {}) {
  ...
  let copyPathBtn: HTMLButtonElement | null = null;
  if (opts.copyPath) {
  ...
  const toolbar = {
    ...
    setPath(text: string) { pathEl.textContent = text; },
