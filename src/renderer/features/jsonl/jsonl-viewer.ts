...
import { marked } from '../../lib/codemirror-setup';
import {
  jsonlViewer, jsonlViewerBody, jsonlViewerSessionId, jsonlViewerTitle, placeholder, terminalArea,
} from '../../lib/dom';
...
function renderToolUse(block: ContentBlock): HTMLElement {
  ...
  if (input.action) {
    try { return renderMcpAction(name, input); } catch {}
  }
  // Default: collapsible JSON
  return toolBlock('#9a9a9c', name, '', makeCollapsible('jsonl-tool-result', 'Input', input, true));
}

function renderMcpAction(name: string, input: ToolInput): HTMLElement {
