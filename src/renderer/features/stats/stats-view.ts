// --- Stats view ---
//
// The stats and usage blobs are produced by the Claude CLI, not by this app, so
// they are read defensively rather than typed to a schema that could drift out
// from under us (see StatsData/UsageData in the preload).
import { escapeHtml } from '../../lib/format';
import { statsViewerBody } from '../../lib/dom';
import type { StatsData } from '../../../domain/stats/stats';
import type { Usage as UsageData } from '../../../domain/usage/usage';

/** date (YYYY-MM-DD) → message count for that day. */
type DailyMap = Record<string, number>;

// The CLI's blobs are open-ended records, so every field is read through one of
// these rather than asserted. A key that is missing, or holds something other
// than what is expected, reads as absent — which is what "defensively" means in
// practice: the chart simply leaves that part out.

function readString(blob: Record<string, unknown>, key: string): string | undefined {
  const value = blob[key];
  return typeof value === 'string' ? value : undefined;
}

function readNumber(blob: Record<string, unknown>, key: string): number | undefined {
  const value = blob[key];
  return typeof value === 'number' && Number.isFinite(value) ? value : undefined;
}

function readRecord(blob: Record<string, unknown>, key: string): Record<string, unknown> {
  const value = blob[key];
...
export async function loadStats() {
  ...
  let stats: StatsData | null | undefined;
  let usage: UsageData;
  try {
  ...
  if (stats) {
    ...
    if (Array.isArray(rawDaily)) {
      ...
      for (const [date, data] of Object.entries(rawDaily)) {
        const d = data as Record<string, number> | number | null;
        dailyMap[date] = typeof d === 'number' ? d : (d?.messageCount || d?.messages || d?.count || 0);
      }
    }
  ...
  if (stats) {
    ...
    const lastDate = readString(stats, 'lastComputedDate') || 'unknown';
...
function buildUsageSection(usage: UsageData): void {
  // Remove existing usage container if present (for refresh)
  const existing = statsViewerBody.querySelector<HTMLElement>('.usage-container');
  ...
  if (usage._rateLimited || usage._error) {
    ...
    const statsNotice = statsViewerBody.querySelector<HTMLElement>('.stats-notice');
  ...
  const items = Array.isArray(usage.limits) && usage.limits.length
    ...
    : [
      ...
      return {
        label: i.label,
        pct: readNumber(flat, i.key),
        reset: readString(flat, i.resetKey) ?? null,
      };
    });
...
function buildDailyBarChart(stats: StatsData): void {
  ...
  if (Array.isArray(rawTokens)) {
    for (const entry of rawTokens) {
      let total = 0;
      for (const count of Object.values(entry.tokensByModel || {})) total += Number(count) || 0;
...
function buildHeatmap(counts: DailyMap): void {
  const container = document.createElement('div');
...
function calculateStreak(counts: DailyMap): { current: number; longest: number } {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
...
function buildStatsSummary(stats: StatsData, dailyMap: DailyMap): void {
  ...
  // Prefer the CLI's own total when it is larger: the daily map only covers the
  // window the heatmap draws, and the CLI counts all of history.
  const reported = readNumber(stats, 'totalMessages');
  if (reported && reported > totalMessages) totalMessages = reported;

  const totalSessions = readNumber(stats, 'totalSessions') || Object.keys(dailyMap).length;

  // Model usage — values are objects with token counts, shown as cards.
  const models = readRecord(stats, 'modelUsage');
