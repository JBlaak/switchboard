...
`;
  ...
  ctl('#sv-save-btn').addEventListener('click', async () => {
    ...
    // Update view.visibleSessionCount, view.sessionMaxAgeDays, and theme
    if (!isProject) {
      if (settings.visibleSessionCount) {
        view.visibleSessionCount = settings.visibleSessionCount;
      }
      if (settings.sessionMaxAgeDays) {
        view.sessionMaxAgeDays = settings.sessionMaxAgeDays;
      }
