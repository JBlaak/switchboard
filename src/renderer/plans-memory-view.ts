...
import { el, jsonlViewer, memoryContent, memoryViewer, placeholder, planViewer, plansContent, settingsViewer, statsViewer, terminalArea } from './dom.js';
import { ICONS } from './icons.js';
import { memoryPanel, planPanel } from './panels.js';
import { state } from './state.js';
import { formatDate } from './utils.js';
import type { MemoryData, MemoryFile, PlanSummary } from '../preload/index.js';

let currentPlanContent = "";
let currentPlanFilePath = "";
let currentPlanFilename = "";
let cachedMemoryData: MemoryData = { global: { files: [] }, projects: [] };
let currentMemoryFilePath: string | null = null;
let currentMemoryContent = "";
const memoryCollapsedState = new Map<string, boolean>();

// --- Plans ---
export async function loadPlans() {
  state.cachedPlans = await window.api.getPlans();
  renderPlans();
}

export function renderPlans(plansArg?: PlanSummary[]): void {
  const plans = plansArg || (state.cachedPlans as PlanSummary[]);
...
async function openPlan(plan: PlanSummary): Promise<void> {
  ...
  plansContent.querySelectorAll<HTMLElement>('.plan-item.active').forEach(el => el.classList.remove('active'));
  const items = plansContent.querySelectorAll<HTMLElement>('.plan-item');
  items.forEach(el => {
    if (el.querySelector<HTMLElement>('.session-id')?.textContent === plan.filename) {
...
export function hidePlanViewer() {
  hideAllViewers();
}
...
export function renderMemories(filterIds?: Set<string> | null): void {
...
function buildMemoryGroup(key: string, label: string, files: MemoryFile[]): HTMLElement {
...
function buildMemoryItem(file: MemoryFile): HTMLElement {
  const item = document.createElement('div');
...
async function openMemory(file: MemoryFile): Promise<void> {
  // Mark active in sidebar
  memoryContent.querySelectorAll<HTMLElement>('.memory-item.active').forEach(el => el.classList.remove('active'));
  const target = memoryContent.querySelector<HTMLElement>(`.memory-item[data-filepath="${CSS.escape(file.filePath)}"]`);
