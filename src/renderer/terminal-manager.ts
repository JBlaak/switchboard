...
import {
  ...
  decodeOsc52Payload, isImeComposing, normalizeTerminalFont, shouldSendSpaceDirectly,
} from './terminal-input.js';
import type { StoredFontSettings } from './terminal-input.js';
import type { OpenSession } from './state.js';
...
import { clearNotifications, setActiveSession, showTerminalHeader, updatePtyTitle } from './app.js';
import { el, gridViewerCount, placeholder, searchBar, searchInput, terminalsEl } from './dom.js';
import { openFileInPanel } from './file-panel.js';
import { focusGridCard, gridCards, handleSessionNavKey, isSessionNavKey, showGridView, toggleGridView, wrapInGridCard } from './grid-view.js';
import { hidePlanViewer } from './plans-memory-view.js';
import { openSessions, remoteStatus, sessionMap, state } from './state.js';
import { TERMINAL_THEME } from './terminal-themes.js';
...
export function createTerminalEntry(session: SessionRow): OpenSession {
  ...
  terminal.onTitleChange(title => {
    ...
    if (state.activeSessionId === entry.session.sessionId) updatePtyTitle();
  });
  return entry;
}
