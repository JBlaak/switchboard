...
//
// Everything below renders a Claude CLI transcript, which is unvalidated data
// written by another program and versioned independently of this app. Rather
// than pretend to a schema that could drift, the entry/content/tool-input shapes
// are typed as open records and every field access stays defensive.
//
// TODO(types): narrow ToolInput per tool once the CLI's transcript format is
// pinned down; the renderers already guard every field they read.
import { marked } from './codemirror-setup.js';
import { el, jsonlViewer, jsonlViewerBody, jsonlViewerSessionId, jsonlViewerTitle, placeholder, terminalArea } from './dom.js';
import { hideAllViewers } from './plans-memory-view.js';
import { escapeHtml } from './utils.js';
import type { SessionRow } from '../shared/types.js';

/** One line of a transcript. Fields are best-effort; nothing here is guaranteed. */
type JsonlEntry = Record<string, any>;
/** A content block inside a message (text, tool_use, tool_result, image, …). */
type ContentBlock = Record<string, any>;
/** A tool call's arguments, whose shape is the tool's business, not ours. */
...
function makeInlineContent(className: string, bodyContent: unknown): HTMLElement {
...
function makeCollapsible(className: string, headerText: string, bodyContent: unknown, startExpanded?: boolean): HTMLElement {
  const wrapper = document.createElement('div');
  wrapper.className = className;
...
function toolBlock(color: string, label: string, summary: string, content?: HTMLElement | null): HTMLElement {
...
function renderToolUse(block: ContentBlock): HTMLElement {
  const name = block.name || 'unknown';
  const input = block.input || {};
  const renderer = (toolRenderers as Record<string, (input: ToolInput, block?: ContentBlock) => HTMLElement>)[name];
  if (renderer) {
...
function renderMcpAction(name: string, input: ToolInput, block?: ContentBlock): HTMLElement {
  const action = input.action;
  // Short display name: strip mcp__ prefix, take last segment
  const shortName = name.replace(/^mcp__/, '').split('__').pop() ?? name;
...
const toolRenderers = {
  ...
  Bash(input: ToolInput) {
    const cmd = input.command || '';
    const pre = document.createElement('pre');
    pre.className = 'jsonl-tool-cmd-block';
    pre.textContent = cmd;
    return toolBlock('#80c0e0', 'Bash', '', pre);
  },

  Grep(input: ToolInput) {
...
function renderLocalCommand({ cmd, output }: { cmd: string; output?: string }): HTMLElement {
  const pre = document.createElement('pre');
  pre.className = 'jsonl-tool-cmd-block';
  pre.textContent = cmd;
  ...
  if (output) {
    let contentEl = el.querySelector<HTMLElement>('.jsonl-tool-content');
    if (!contentEl) {
...
function mergeLocalCommandEntries(entries: JsonlEntry[]): JsonlEntry[] {
  const result = [];
  let i = 0;
...
function getEntryText(entry: JsonlEntry): string | null {
...
function mergeLocalCommandBlocks(blocks: ContentBlock[]): ContentBlock[] {
...
function renderToolResult(resultData: any, container: HTMLElement): void {
...
function extractResultText(data: any): string | null {
  if (!data) return null;
  if (typeof data === 'string') {
...
function renderJsonlEntry(entry: JsonlEntry, toolResultMap: Map<string, any>): HTMLElement | null {
  ...
  for (const block of contentBlocks) {
    if (block.type === 'thinking' && block.thinking) {
      ...
      if (block.id && toolResultMap && toolResultMap.has(block.id)) {
        ...
        let contentEl = toolEl.querySelector<HTMLElement>('.jsonl-tool-content');
        if (!contentEl) {
          contentEl = document.createElement('div');
          contentEl.className = 'jsonl-tool-content';
          toolEl.appendChild(contentEl);
        }
        renderToolResult(resultData, contentEl as HTMLElement);
      }
...
export async function showJsonlViewer(session: SessionRow): Promise<void> {
  ...
  if (!result || result.error) {
    jsonlViewerBody.innerHTML = '<div class="plans-empty">Error loading messages: ' + escapeHtml(result?.error ?? 'unknown error') + '</div>';
    return;
  }

  const rawEntries = (result.entries || []) as JsonlEntry[];
  ...
  jsonlViewerBody.querySelectorAll<HTMLImageElement>('.jsonl-clickable-img').forEach(img => {
    img.onclick = () => {
      const overlay = document.createElement('div');
      overlay.className = 'jsonl-screenshot-fullscreen';
      const fullImg = document.createElement('img');
