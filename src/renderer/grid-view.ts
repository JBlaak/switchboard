...
//
import { clearNotifications, confirmAndStopSession, setActiveSession, updateRunningIndicators } from './app.js';
import { el, gridViewer, gridViewerCount, jsonlViewer, memoryViewer, placeholder, planViewer, settingsViewer, sidebarContent, statsViewer, terminalArea, terminalHeader, terminalsEl } from './dom.js';
import { openSessions, sessionMap, state } from './state.js';
import { fitAndScroll, isMac, showSession } from './terminal-manager.js';
...
export function wrapInGridCard(sessionId: string): void {
  ...
  stopBtn.style.display = state.activePtyIds.has(sessionId) ? '' : 'none';
  ...
  entry.element.addEventListener('focusin', () => {
    if (state.gridViewActive && gridFocusedSessionId !== sessionId) {
      focusGridCard(sessionId);
    }
  });
...
export function showGridView() {
  state.gridViewActive = true;
  localStorage.setItem('state.gridViewActive', '1');
  ...
  requestAnimationFrame(() => {
    const toFocus = state.activeSessionId && sessionIds.includes(state.activeSessionId) ? state.activeSessionId : sessionIds[0];
...
function hideGridView() {
  ...
  localStorage.setItem('state.gridViewActive', '0');
  unwrapGridCards();
...
export function toggleGridView() {
  if (state.gridViewActive) {
    const restoreId = gridFocusedSessionId || state.activeSessionId;
    hideGridView();
...
function navigateSession(direction: -1 | 1): void {
  ...
  const current = state.gridViewActive ? gridFocusedSessionId : state.activeSessionId;
...
function navigateGrid(direction: GridDirection): void {
  ...
  const currentCard = gridCards.get(gridFocusedSessionId || state.activeSessionId || '');
