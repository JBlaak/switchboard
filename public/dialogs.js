...
async function launchRemoteSession(project, kind) {
  const sessionId = crypto.randomUUID();
  const projectPath = project.projectPath;
  const session = {
    sessionId,
...
function showAddProjectDialog() {
  ...
  let mode = 'local';
  dialog.querySelectorAll('.add-project-tab').forEach(tab => {
    tab.onclick = () => {
      mode = tab.dataset.mode;
      dialog.querySelectorAll('.add-project-tab').forEach(t => t.classList.toggle('active', t === tab));
      dialog.querySelector('#add-project-local').style.display = mode === 'local' ? '' : 'none';
      dialog.querySelector('#add-project-remote').style.display = mode === 'remote' ? '' : 'none';
      errorEl.style.display = 'none';
      (mode === 'local' ? pathInput : dialog.querySelector('#add-remote-user')).focus();
    };
  });
  ...
  async function addProject() {
    ...
    if (mode === 'remote') {
      const user = dialog.querySelector('#add-remote-user').value.trim();
      const host = dialog.querySelector('#add-remote-host').value.trim();
      const port = dialog.querySelector('#add-remote-port').value.trim();
      const dir = dialog.querySelector('#add-remote-dir').value.trim();
      if (!user || !host) {
        showError('Please enter a username and host.');
        return;
      }
      const result = await window.api.addRemoteProject({ user, host, port: port || 22, dir });
      if (result.error) {
