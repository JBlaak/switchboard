...
import * as sessionCache from '../src/main/session-cache.js';
import type { CacheDb } from '../src/main/session-cache.js';
import type { CachedSession, FolderMeta } from '../src/shared/types.js';
...
function makeFakeDb(metaMap: Map<string, FolderMeta>): { indexedFolders: Set<string>; db: CacheDb } {
  const indexedFolders = new Set<string>();
  const db: CacheDb = {
    deleteCachedFolder() {},
    getCachedByFolder() { return []; },
    upsertCachedSessions(sessions: CachedSession[]) { for (const s of sessions) indexedFolders.add(s.folder); },
    deleteCachedSession() {},
    deleteSearchFolder() {},
    deleteSearchSession() {},
    upsertSearchEntries() {},
    ...
    getAllCached() { return []; },
    getSetting() { return {} as never; },
    getMeta() { return null; },
    setName() {},
  };
  return { indexedFolders, db };
}
